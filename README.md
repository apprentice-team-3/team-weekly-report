...
- [ ] メンバータスク一覧画面
    - [ ] ユーザーが所属しているチームメンバー全員のプロジェクトのタスクを1週間(7つ)単位で表示する。
        - [ ] 並び順を追加する
            - [ ] 評価待ち順
            - [ ] 新着順
            - [ ] 【advanced】HELP順、コメント新着順 (他のサービスとの差別化)
        - [ ] ユーザー名を表示する
            - [ ]【advanced】ユーザー名の左に画像を表示する
        - [ ] それぞれのタスクについて
            - [ ] 日付を表示する
            - 本日の日付のカラムの場合
                - [ ] タスクを追加ボタンを表示する
            - 本日の日付以外のカラムの場合
                ...
                - [ ] プログレスバーを表示する
                - [ ] 1日の親タスクは3件まで表示する
                    - [ ] 3件以上の場合はスクロールで閲覧できるようにする
                - [ ] 親タスクをクリックすると、タスク詳細画面へジャンプ

    - [ ] プロジェクト全体の進行度を表示する
        - ユーザーごとの評価待ちのタスクの総数 / ユーザーごとのタスクの総数 * 100

    - [ ] ページネーションを表示する
        - [ ] ページネーションをクリックすると適切にページが更新される
            - [ ] 新着順
            - [ ] 評価待ち順
...
- [ ] 評価一覧画面
    ...
    - [ ] ユーザーごとに対象の週に評価された軸が多い順で3種類の評価軸ボタンが表示する。
    - [ ] 評価軸ボタンをクリックすると評価詳細ポップアップを表示する
    - 評価詳細ポップアップ
        - [ ] ユーザーごとの評価された親タスク子タスクを表示する
        - [ ] メンバーのコメントを表示ボタンを表示する
            - [ ] メンバーのコメント表示ボタンをクリックすると、評価した際のコメントが表示される
    - [ ] ページネーションを表示する
